%% This function defines an EM algorithm using ER-CHMM structure that we use in section 5 of our project.

%% Sources used for this algorithm:

% The procedure that we use is based off the algorithms detailed in the
% following sources: 
...
function [D0, D1, logli] = EM_algorithm_using_ER_CHMM(trace, orders)
    ...
    %% Define a function to print the different Erlang numbers for when we run the algorithm.
...
%% Define process that runs the algorithm with lots of different orders. We then choose the algorithm with the best logli function.  
    ...
    if length(orders)==1
        ...
        if show_progress
            fprintf('Best solution: logli=%g, orders=', log_li_best);
    ...
    while abs((ologli-logli)/logli)>stop_condition && steps<=max_iterations
        ologli = logli;
        % E-step:
        ...
        logli = (log(likelihood_value) + B_likelihoods_scale(1) * log(2)) / K;
        ...
        if show_progress && etime(clock(),t1)>2
    ...
    if show_progress
        fprintf('Num of iterations: %d, logli: %g\n', steps, logli);
        fprintf('EM algorithm ended. (orders=');
        for i=1:M
            fprintf('%g',orders(i));
            if i<M
                fprintf(',');
